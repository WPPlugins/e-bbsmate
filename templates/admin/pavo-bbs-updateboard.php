...
//게시판 ID
$post_id = sanitize_text_field( $_POST['ebbsmate_update_board_id'] );

//nonce_check
if ( empty( $_POST['ebbsmate_board_nonce'] ) || empty( $_POST['ebbsmate_update_board_id'] ) || ( ! wp_verify_nonce( $_POST['ebbsmate_board_nonce'], 'ebbsmate_editboard_'.$post_id ) ) ) {
	echo "<script>
			alert('잘못된 접근입니다.');
			window.history.back()
		</script>";
...
$board_role_notice = array_map( 'esc_attr', $board_role_write );
...
//게시판 권한 설정 저장
$user_logins = explode(",", $board_admin_ids);
...
////////////////////////
//게시판 활성화 여부
update_post_meta($post_id, 'ebbsmate_status_flag', 1);

//권한 설정 - 목록 보기
update_post_meta($post_id, 'ebbsmate_list_role', $board_role_list);

//권한 설정 - 게시글 읽기
update_post_meta($post_id, 'ebbsmate_read_role', $board_role_read);

//권한 설정 - 게시글 쓰기
update_post_meta($post_id, 'ebbsmate_write_role', $board_role_write);

//권한 설정 - 댓글 쓰기
update_post_meta($post_id, 'ebbsmate_comment_role', $board_role_comment);

//권한 설정 - 공지글 쓰기
update_post_meta($post_id, 'ebbsmate_notice_role', $board_role_notice);

//권한 추가 - 게시판 관리자(CUSTOM USER ROLE)
update_post_meta($post_id, 'ebbsmate_admin_ids', $board_admin_ids);

//부가 기능 - 첨부파일 사용 여부
update_post_meta($post_id, 'ebbsmate_attach_flag', $board_file_attach_flag);

//부가 기능 - 첨부파일 최대 용량
update_post_meta($post_id, 'ebbsmate_attach_size', $board_file_attach_size);

//부가 기능 - 첨부파일 최대 개수
update_post_meta($post_id, 'ebbsmate_attach_item', $board_file_attach_item);

//부가 기능 - 최상단 공지글
update_post_meta($post_id, 'ebbsmate_notice_flag', $board_notice_flag);

//부가 기능 - 금칙어 필터링
update_post_meta($post_id, 'ebbsmate_word_filtering', $board_word_filtering_flag);

//부가 기능 - 비밀글
update_post_meta($post_id, 'ebbsmate_secret_post', $board_secret_post);

//부가 기능 - 이전글/다음글
update_post_meta($post_id, 'ebbsmate_prev_next_post', $board_prev_next_post);

//부가 기능 - 이전글/다음글 개수
update_post_meta($post_id, 'ebbsmate_prev_next_lines', $board_prev_next_lines);

//레이아웃 및 스타일 - 목록 글수
update_post_meta($post_id, 'ebbsmate_list_lines', $board_list_lines);

//레이아웃 및 스타일 - 스타일
update_post_meta($post_id, 'ebbsmate_css_style', $board_css_style);

//게시글 항목 설정 - 작성자
update_post_meta($post_id, 'ebbsmate_author_flag', $author_flag);

//게시글 항목 설정 - 작성일
update_post_meta($post_id, 'ebbsmate_date_flag', $date_flag);

//게시글 항목 설정 - 조회수
update_post_meta($post_id, 'ebbsmate_vcount_flag', $vcount_flag);

//항목별 권한 설정 여부
update_post_meta($post_id, 'ebbsmate_section_flag', $board_section_flag);

//항목별 권한
update_post_meta($post_id, 'ebbsmate_section', $board_section);

//머릿말 사용여부
//update_post_meta($post_id, 'ebbsmate_preface_flag', $board_preface_flag);

$edit_nonce = wp_create_nonce( 'ebbsmate_editboard_'.$post_id );

echo "<script> window.location='".admin_url('admin.php?page=ebbsmate_board&action=editboard&board_id='.$post_id.'&settings-updated=true&ebbsmate_board_nonce='.$edit_nonce)."';</script>";
?>